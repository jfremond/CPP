...
/*   Updated: 2023/03/28 19:45:25 by jfremond         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	BitcoinExchange::_checkDateValidity(std::string date)
{
	...
	if (_error == 0 && (_stod(year) < 2009 || _stod(year) > 2022))
		_error = 2;
	if (_error == 0 && (_stod(mon) < 01 || _stod(mon) > 12))
		_error = 2;
	if (_error == 0 && (_stod(day) < 01 || _stod(day) > 31))
		_error = 2;
...
void	BitcoinExchange::_printRes(int error, std::string date, std::string value)
{
	switch (error)
	{
		...
		case 5:
			...
			break;	
		default:
			std::cout << date << " => " << value << " = " << std::setprecision(date.length()) << _stod(value) * _it->second << std::endl;
			break;
	}
}
